...
const getPendingBookings = async (req, res) => {
  try {
    ...
    console.error(
      "Lỗi khi lấy danh sách đặt phòng cần xác nhận:",
      error.message
    );
...
const getBookingDetails = async (req, res) => {
  try {
    const response = await bookingService.getBookingDetailsService(
      req.params.id,
      req.user._id
    );
    return res.status(200).json(response);
...
const updateBookingStatus = async (req, res) => {
  try {
    ...
    const response = await bookingService.updateBookingStatusService(
      req.params.id,
      req.user._id,
      status
    );
...
const getCustomerBookingDetail = async (req, res) => {
  try {
    ...
    if (!mongoose.Types.ObjectId.isValid(bookingId)) {
      return res
        .status(400)
        .json({ success: false, message: "ID đặt phòng không hợp lệ" });
    }
