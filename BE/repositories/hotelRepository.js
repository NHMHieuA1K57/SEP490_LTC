...
const HotelRepository = {
  ...
  getHotelDetails: async (hotelId, filters = {}) => {
    ...
    if (checkInDate && checkOutDate && numberOfPeople) {
      ...
      if (!isNaN(checkIn) && !isNaN(checkOut) && numberOfPeople > 0) {
        roomQuery.availability = {
          $elemMatch: { date: { $gte: checkIn, $lte: checkOut }, quantity: { $gte: parseInt(numberOfPeople) } },
        };
    ...
    const reviews = await Review.find({ hotelId }).select('userId comment createdAt').lean();
    ...
    return {
      basicInfo: { name: hotel.name || '', address: hotel.address || '', location: { latitude: hotel.location.coordinates[1] || 0, longitude: hotel.location.coordinates[0] || 0 }, description: hotel.description || '' },
      images: hotel.images || [],
      amenities: hotel.amenities || [],
      rating: { average: hotel.rating || 0, reviewCount: hotel.reviewCount || reviews.length },
      rooms: hotel.rooms ? hotel.rooms.map(room => ({
        ...
        availability: room.availability ? room.availability.map(slot => ({ date: slot.date, available: slot.quantity > 0, quantity: slot.quantity })) : [],
        description: room.description || '',
        images: room.images || [],
      })) : [],
      reviews: reviews.map(review => ({ username: userMap[review.userId.toString()] || 'Khách ẩn danh', content: review.comment || '', date: review.createdAt || new Date() })),
      policies: {
      ...
      contact: { phone: hotel.additionalInfo?.contact?.phone || '', email: hotel.additionalInfo?.contact?.email || '', website: hotel.additionalInfo?.contact?.website || '' },
    };
  },
  ...
  deleteHotel: async (hotelId, ownerId) => {
    const activeBookings = await Booking.countDocuments({ hotelId, status: { $in: ['pending', 'confirmed'] } });
    if (activeBookings > 0) throw new Error('Không thể xóa khách sạn vì còn đặt phòng đang hoạt động');
    return await Hotel.findOneAndDelete({ _id: hotelId, ownerId });
  },

  searchHotelsHandler: async (filters, page = 1, limit = 10) => {
    const {
      location, checkInDate, checkOutDate, numberOfPeople, roomType, minPrice, maxPrice, amenities, category, minRating, maxRating, hasPromotion,
    } = filters;
    ...
    if (amenities) {
      ...
      if (Array.isArray(amenitiesArray) && amenitiesArray.length) query.amenities = { $all: amenitiesArray };
    }
    if (category) {
    ...
    if (hasPromotion) roomQuery.promotion = { $exists: true, $ne: null };
    if (checkInDate && checkOutDate && numberOfPeople) {
      const checkIn = new Date(checkInDate);
      const checkOut = new Date(checkOutDate);
      if (!isNaN(checkIn) && !isNaN(checkOut) && numberOfPeople > 0) {
        roomQuery.availability = {
          $elemMatch: { date: { $gte: checkIn, $lte: checkOut }, quantity: { $gte: parseInt(numberOfPeople) || 1 } },
        };
      }
    }
    ...
    if (Object.keys(roomQuery).length > 0) query._id = { $in: matchingHotelIds };
    ...
    const [hotels, total] = await Promise.all([
      Hotel.find(query)
        ...
        .populate({ path: 'rooms', match: roomQuery })
        .skip(skip)
        .limit(limit)
        .lean(),
    ...
    return { items: hotels, total, page, totalPages: Math.ceil(total / limit) };
  },

  getAllHotels: async () => 
    await Hotel.find()
      .populate({ path: 'rooms', select: 'price' })
