...
const AddRoom = () => {
  ...
  return (
    <form>
        <Title align='left' font='outfit' title='Add Room' subTitle='Add a new room to your hotel' />
        <p className='text-gray-800 mt-10'>Images</p>
        <div className='grid grid-cols-2 sm:flex gap-4 my-2 flex-wrap'>
            {Object.keys(images).map((key) => (
                <label htmlFor={`roomImage${key}`} key={key}>
                    <img className='max-h-13 cursor-pointer opacity-80' src={images[key] ? URL.createObjectURL(images[key]) : ""} alt="" />
        ...
        <div className='w-full flex max-sm:flex-col sm:gap-4 mt-4'>
            ...
            <div>
                ...
                <inut type="number" placeholder='0' className='border border-gray-300 mt-1 rounded p-2 w-24' value={inputs.pricePerNight} onChange={e=> setInputs({...inputs, pricePerNight: e.target.value})} />
            </div>
        </div>
        ...
        <button className='bg-primary text-white py-2 px-8 rounded mt-8'>
            Add Room
        </button>
    </form>
  )
